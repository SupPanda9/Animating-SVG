...
function enableElementDrag() {
    ...
    function dragStart(event) {
        ...
        if (elementType === 'rect' || elementType === 'text') {
            originalX = parseFloat(selectedElement.getAttribute('x')) || 0;
            originalY = parseFloat(selectedElement.getAttribute('y')) || 0;
        } else if (elementType === 'circle' || elementType === 'ellipse') {
            originalX = parseFloat(selectedElement.getAttribute('cx')) || 0;
            originalY = parseFloat(selectedElement.getAttribute('cy')) || 0;
    ...
    function dragMove(event) {
        ...
        if (elementType === 'rect' || elementType === 'text') {
            ...
            const bbox = selectedElement.getBBox();
            const deltaX = dx - originalX;
            const deltaY = dy - originalY;
            translatePathElement(selectedElement, deltaX, deltaY);
            originalX = dx;
            originalY = dy;
        }
    }
...
function translatePathElement(element, dx, dy) {
    const d = element.getAttribute('d');
    const newD = d.replace(/(-?\d+(\.\d+)?)/g, (match, p1, p2, offset) => {
        const isXCoord = offset % 2 === 0;
        const number = parseFloat(match);
        return isXCoord ? number + dx : number + dy;
    });
